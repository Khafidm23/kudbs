...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$conn) {
        header("Location: log.php?error=" . urlencode("Connection failed: " . mysqli_connect_error()));
        exit;
    }
    ...
    if (mysqli_query($conn, $delete_query)) {
        ...
        header("Location: log.php?message=" . urlencode("All records have been successfully deleted"));
        exit;
    } else {
        // Jika gagal, arahkan kembali ke halaman log dengan pesan error
        header("Location: log.php?error=" . urlencode("Failed to delete records: " . mysqli_error($conn)));
        exit;
    }
}
