...
$query_tagihan = "
    ...
    FROM (
        SELECT 'BS 01' AS sumber, pengguna, no_hp, jumlah_hm, harga_hm, total_tagihan, total_angsuran, sisa_tagihan FROM tb_tgbs01 WHERE pengguna = '$pengguna' AND no_hp = (
        SELECT no_hp FROM $table WHERE id = '$id'
    )
        UNION ALL
        SELECT 'BS 02' AS sumber, pengguna, no_hp, jumlah_hm, harga_hm, total_tagihan, total_angsuran, sisa_tagihan FROM tb_tgbs02 WHERE pengguna = '$pengguna' AND no_hp = (
        SELECT no_hp FROM $table WHERE id = '$id'
    )
        UNION ALL
        SELECT 'BS 03' AS sumber, pengguna, no_hp, jumlah_hm, harga_hm, total_tagihan, total_angsuran, sisa_tagihan FROM tb_tgbs03 WHERE pengguna = '$pengguna' AND no_hp = (
        SELECT no_hp FROM $table WHERE id = '$id'
    )
        UNION ALL
        SELECT 'BS 04' AS sumber, pengguna, no_hp, jumlah_hm, harga_hm, total_tagihan, total_angsuran, sisa_tagihan FROM tb_tgbs04 WHERE pengguna = '$pengguna' AND no_hp = (
        SELECT no_hp FROM $table WHERE id = '$id'
    )
        UNION ALL
        SELECT 'BS 05' AS sumber, pengguna, no_hp, jumlah_hm, harga_hm, total_tagihan, total_angsuran, sisa_tagihan FROM tb_tgbs05 WHERE pengguna = '$pengguna' AND no_hp = (
        SELECT no_hp FROM $table WHERE id = '$id'
    )
    ) AS combined
...
$query_angsuran = "
    SELECT pengguna, no_hp, tanggal, angsuran, sumber
    FROM tb_angsuran 
                        ...
                        if (mysqli_num_rows($result_angsuran) > 0) {
                            while ($data_angsuran = mysqli_fetch_assoc($result_angsuran)) { ?>
                                <tr>
                                    ...
                                    <td><?= htmlspecialchars($data_angsuran['sumber']) ?></td>
                                </tr>
                            <?php }
                        } else { ?>
                            <tr>
