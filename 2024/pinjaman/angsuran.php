...
if (isset($_GET['table'])) {
    ...
    // Validasi nama tabel
    $allowedTables = ['tb_angsuran_pinjaman'];
    ...
    // Query angsuran
    $query_angsuran = "
    SELECT tanggal, nama, no_hp, angsuran, keterangan
    FROM `$table` 
    ...
    $sheet->setCellValue('D1', 'Jumlah Angsuran');
    ...
    if (mysqli_num_rows($result_angsuran) > 0) {
        while ($data_angsuran = mysqli_fetch_assoc($result_angsuran)) {
            ...
            $sheet->setCellValue('B' . $row, $data_angsuran['nama']);
            $sheet->setCellValue('C' . $row, $data_angsuran['no_hp']);
            $sheet->setCellValue('D' . $row, $data_angsuran['angsuran']);
            $sheet->setCellValue('E' . $row, $data_angsuran['keterangan']);
            $row++;
        }
    }
