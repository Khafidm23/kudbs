...
if (isset($_POST['upload'])) {
    ...
    // Memastikan tabel yang dipilih valid
    $validTables = ['tb_hmbs01', 'tb_hmbs02', 'tb_hmbs03', 'tb_hmbs04', 'tb_hmbs05'];
    ...
    if (isset($_FILES['file']) && $_FILES['file']['error'] == 0) {
        ...
        try {
            ...
            for ($row = 2; $row <= $highestRow; $row++) { // Mulai dari baris kedua untuk mengabaikan header
                ...
                $pengguna = $sheet->getCell('B' . $row)->getValue();
                $no_hp = $sheet->getCell('C' . $row)->getValue();
                $hm_awal = $sheet->getCell('D' . $row)->getValue();
                $hm_akhir = $sheet->getCell('E' . $row)->getValue();
                $jumlah_hm = $sheet->getCell('F' . $row)->getValue();
                $harga_hm = $sheet->getCell('G' . $row)->getValue();
                $keterangan = $sheet->getCell('H' . $row)->getValue();

                // Mengabaikan baris jika kolom penting kosong (misalnya, pengguna)
                if (empty($pengguna) || empty($no_hp)) {
                ...
                $query = "
                    INSERT INTO $table (id, tanggal, pengguna, no_hp, hm_awal, hm_akhir, jumlah_hm, harga_hm, keterangan)
                    VALUES ('$uuid', '$tanggal', '$pengguna', '$no_hp', '$hm_awal', '$hm_akhir', '$jumlah_hm', '$harga_hm', '$keterangan')
                ";
        ...
        <form action="" method="post" enctype="multipart/form-data">
            <div class="form-group">
                ...
                <select name="table" id="table" required>
                    ...
                    <option value="tb_hmbs01">Tabel HMBS 01</option>
                    <option value="tb_hmbs02">Tabel HMBS 02</option>
                    <option value="tb_hmbs03">Tabel HMBS 03</option>
                    <option value="tb_hmbs04">Tabel HMBS 04</option>
                    <option value="tb_hmbs05">Tabel HMBS 05</option>
                </select>
            </div>
