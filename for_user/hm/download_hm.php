...
if (isset($_GET['table'])) {
    ...
    // Query hm
    $query_hm = "
    SELECT tanggal, pengguna, hm_awal, hm_akhir, jumlah_hm, harga_hm, keterangan
    FROM `$table` 
    ...
    if (mysqli_num_rows($result_hm) > 0) {
        while ($data_hm = mysqli_fetch_assoc($result_hm)) {
            ...
            $sheet->setCellValue('C' . $row, $data_hm['hm_awal']);
            $sheet->setCellValue('D' . $row, $data_hm['hm_akhir']);
            $sheet->setCellValue('E' . $row, $data_hm['jumlah_hm']);
            $sheet->setCellValue('F' . $row, $data_hm['harga_hm']);
            $sheet->setCellValue('G' . $row, $data_hm['keterangan']);
            $row++;
        }
    }
