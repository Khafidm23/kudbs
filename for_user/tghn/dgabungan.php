...
if (isset($_GET['table'])) {
    ...
    $sheet->setCellValue('B1', 'Total Tagihan');
    $sheet->setCellValue('C1', 'Total Angsuran');
    $sheet->setCellValue('D1', 'Sisa Tagihan');
    ...
    if (mysqli_num_rows($result_tagihan) > 0) {
        while ($data_tagihan = mysqli_fetch_assoc($result_tagihan)) {
            ...
            $sheet->setCellValue('B' . $row, $data_tagihan['total_tagihan']);
            $sheet->setCellValue('C' . $row, $data_tagihan['total_angsuran']);
            $sheet->setCellValue('D' . $row, $data_tagihan['sisa_tagihan']);
            $row++;
        }
    }
