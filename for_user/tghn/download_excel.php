...
if (isset($_GET['pengguna']) && isset($_GET['id']) && isset($_GET['table'])) {
    ...
    $sheet->setCellValue('D1', 'Jumlah HM');
    ...
    if (mysqli_num_rows($result_hm) > 0) {
        while ($data_hm = mysqli_fetch_assoc($result_hm)) {
            ...
            $sheet->setCellValue('C' . $row, $data_hm['tanggal']);
            $sheet->setCellValue('D' . $row, $data_hm['jumlah_hm']);
            $sheet->setCellValue('E' . $row, $data_hm['harga_hm']);
            $row++;
        }
    }
    ...
    $sheet->setCellValue('C' . $row, 'Angsuran');
    ...
    if (mysqli_num_rows($result_angsuran) > 0) {
        while ($data_angsuran = mysqli_fetch_assoc($result_angsuran)) {
            ...
            $sheet->setCellValue('B' . $row, $data_angsuran['tanggal']);
            $sheet->setCellValue('C' . $row, $data_angsuran['angsuran']);
            $row++;
        }
    }
    ...
    $sheet->setCellValue('A' . $row, 'Nama');
    $sheet->setCellValue('B' . $row, 'Jumlah HM');
    $sheet->setCellValue('C' . $row, 'Harga HM');
    $sheet->setCellValue('D' . $row, 'Total Tagihan');
    $sheet->setCellValue('E' . $row, 'Total Angsuran');
    $sheet->setCellValue('F' . $row, 'Sisa Tagihan');
    ...
    if (mysqli_num_rows($result_tagihan) > 0) {
        while ($data_tagihan = mysqli_fetch_assoc($result_tagihan)) {
            ...
            $sheet->setCellValue('B' . $row, $data_tagihan['jumlah_hm']);
            $sheet->setCellValue('C' . $row, $data_tagihan['harga_hm']);
            $sheet->setCellValue('D' . $row, $data_tagihan['total_tagihan']);
            $sheet->setCellValue('E' . $row, $data_tagihan['total_angsuran']);
            $sheet->setCellValue('F' . $row, $data_tagihan['sisa_tagihan']);
            $row++;
        }
    }
