...
if (isset($_GET['table'])) {
    ...
    $sheet->setCellValue('E1', 'Jumlah Angsuran');
    ...
    if (mysqli_num_rows($result_angsuran) > 0) {
        while ($data_angsuran = mysqli_fetch_assoc($result_angsuran)) {
            ...
            $sheet->setCellValue('C' . $row, $data_angsuran['sumber']);
            $sheet->setCellValue('D' . $row, $data_angsuran['harga_hm']);
            $sheet->setCellValue('E' . $row, $data_angsuran['angsuran']);
            $sheet->setCellValue('F' . $row, $data_angsuran['keterangan']);
            $row++;
        }
    }
