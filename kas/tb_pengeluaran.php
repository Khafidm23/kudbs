...
// Mulai query SQL
$sql_pengeluaran = "SELECT * FROM tb_pengeluaran";
$conditions = [];

// Jika tombol cari ditekan, gunakan pencarian tanpa filter bulan dan tahun
if ($is_search) {
    $conditions[] = "(tanggal LIKE '%$search_keyword%' OR keterangan LIKE '%$search_keyword%')";
} else {
...
if (count($conditions) > 0) {
    $sql_pengeluaran .= " WHERE " . implode(" AND ", $conditions);
}

// Hitung total data
$sql_total = str_replace('SELECT *', 'SELECT COUNT(*) AS total', $sql_pengeluaran);
$total_result = mysqli_query($conn, $sql_total);
$total_data = mysqli_fetch_assoc($total_result)['total'];
$total_pages = ceil($total_data / $limit);

// Tambahkan urutan dan batasan
$sql_pengeluaran .= " ORDER BY $order_by $order_direction LIMIT $limit OFFSET $offset";
$result = mysqli_query($conn, $sql_pengeluaran);
?>

<div class="card mt-3 shadow-sm">
    <div class="card-header bg-info text-light"><b>Rekapan Pengeluaran Kas</b></div>
    <div class="card-body">
        <div class="d-flex justify-content-between">
        <div><a href="download_pengeluaran.php?table=tb_pengeluaran" class="btn btn-success">Download Daftar Pengeluaran</a></div>
                    ...
                    <div class="d-flex justify-content-between mb-3">
                        ...
                        <div>
                            <button class="btn btn-secondary" name="tampilkan_semua" type="submit">Tampilkan
                                Semua</button>
                                <a href="add2.php?table=tb_pengeluaran" class="btn btn-success">Tambah pengeluaran</a>
                        </div>
                    </div>
                </form>
            </div>
        ...
        <form method="POST" action="del_all.php" onsubmit="return confirmDeleteAll()">
            <input type="hidden" name="table" value="tb_pengeluaran">
            <input type="hidden" name="redirect" value="tb_pengeluaran.php">
                ...
                <table class="table table-striped table-hover table-bordered">
                    <thead class="table-header table-info">
                        <tr>
                            ...
                            <th>Keterangan</th>
                            <th style="width: 250px;">Jumlah pengeluaran</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td style="vertical-align: middle;"><?= $data['pengeluaran'] ?></td>
                            <td>
                                <a href="edit2.php?id=<?= $data['id'] ?>&table=tb_pengeluaran"
                                    class="btn btn-warning">Edit</a>
                                <a href="delete.php?id=<?= $data['id'] ?>&table=tb_pengeluaran" class="btn btn-danger"
