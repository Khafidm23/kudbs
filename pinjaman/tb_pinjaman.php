...
// Mulai query SQL
$sql_pinjaman = "SELECT * FROM tb_pinjaman";
$conditions = [];

// Jika tombol cari ditekan, gunakan pencarian tanpa filter bulan dan tahun
if ($is_search) {
    $conditions[] = "(nama LIKE '%$search_keyword%' OR tanggal LIKE '%$search_keyword%' OR keterangan LIKE '%$search_keyword%')";
} else {
...
if (count($conditions) > 0) {
    $sql_pinjaman .= " WHERE " . implode(" AND ", $conditions);
}

// Hitung total data
$sql_total = str_replace('SELECT *', 'SELECT COUNT(*) AS total', $sql_pinjaman);
$total_result = mysqli_query($conn, $sql_total);
$total_data = mysqli_fetch_assoc($total_result)['total'];
$total_pages = ceil($total_data / $limit);

// Tambahkan urutan dan batasan
$sql_pinjaman .= " ORDER BY $order_by $order_direction LIMIT $limit OFFSET $offset";
$result = mysqli_query($conn, $sql_pinjaman);
?>

<div class="card mt-3 shadow-sm">
    <div class="card-header bg-info text-light">Rekapan Pinjaman Karyawan</div>
    <div class="card-body">
        <div class="d-flex justify-content-between">
        <div><a href="pinjaman.php?table=tb_pinjaman" class="btn btn-success">Download Daftar Pinjaman</a></div>
            ...
            <div class="card p-3 mb-3 shadow-sm">
                <form method="POST" style="overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
                    ...
                    <div class="d-flex justify-content-between mb-3">
                        ...
                        <div>
                            <a href="add2.php?table=tb_pinjaman" class="btn btn-success">Tambah Pinjaman</a>
                        </div>
                    </div>
                </form>
            </div>
        ...
        <form method="POST" action="del_all.php" onsubmit="return confirmDeleteAll()">
            <input type="hidden" name="table" value="tb_pinjaman">
            <input type="hidden" name="redirect" value="tb_pinjaman.php">
                ...
                <table class="table table-striped table-hover table-bordered">
                    <thead class="table-header table-info">
                        <tr>
                            ...
                            <th style="vertical-align: middle;">Jumlah Pinjaman</th>
                            <th style="vertical-align: middle;">Keterangan</th>
                            <th style="width: 170px; vertical-align: middle;">Aksi</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php 
                        ...
                        <tr>
                            ...
                            <td style="vertical-align: middle; width: 250px;"><?= $data['pinjaman'] ?></td>
                            <td style="vertical-align: middle;"><?= $data['keterangan'] ?></td>
                            <td style="vertical-align: middle;">
                                <a href="edit2.php?id=<?= $data['id'] ?>&table=tb_pinjaman"
                                    class="btn btn-warning">Edit</a>
                                <a href="delete.php?id=<?= $data['id'] ?>&table=tb_pinjaman" class="btn btn-danger"
